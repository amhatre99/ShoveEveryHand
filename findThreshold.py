...
def find_threshold(p, starting_stack, trials):
    ...
    for _ in range(trials):
        ...
        while 0 < hero_money < 2 * starting_stack:
            ...
            if strHand[1] != strHand[3]:
                strHand = ''.join(sorted([strHand[0], strHand[2]], key= rank_key, reverse=True)) + 'o'
            else:
                strHand = ''.join(sorted([strHand[0], strHand[2]], key= rank_key, reverse=True)) + 's'
            if final_lookup[strHand] < p:
...
if __name__ == '__main__':
    ...
    precision = 0.01
    while curr < high:
        winrate = find_threshold(curr, starting_stack, trials)
        if winrate > maximum:
            maximum = winrate
            best_prob = curr
        curr += precision
